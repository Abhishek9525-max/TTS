...
// // speechify-backend-server.js (with REST endpoint for Postman testing)
...
// // REST endpoint for testing in Postman
// app.post("/tts", async (req, res) => {
//   try {
//     const {
//       text,
//       voiceId,
//       modelId = "eleven_multilingual_v2",
//       languageCode, // default to Hindi
//     } = req.body;

//     if (!text || !voiceId)
//       return res.status(400).json({ error: "text and voiceId required" });

//     // Call ElevenLabs streaming API with languageCode
//     const stream = await client.textToSpeech.stream(voiceId, {
//       text,
//       modelId,
...
//     // Save MP3 in audio_files folder
//     const filePath = path.join(AUDIO_DIR, `audio_${Date.now()}.mp3`);
//     fs.writeFileSync(filePath, audioBuffer);

//     res.json({ message: "Audio saved locally", path: filePath });
//   } catch (err) {
//     console.error(err);
//     res.status(500).json({ error: "TTS generation failed" });
//   }
